...
cf.read(os.path.join(BASE_DIR,'info.conf'))
class gettrace:
    ...
    def show(self):
        try:
            ...
            if self.rstr:
                    ...
                    f.write(self.rstr+"\n")
                    f.close()    
                    self.orireuslt=self.url+"   "+self.ip+"    "+str(self.time1)+"    "+str(time2)+"\n"+self.rstr+"\n"
                    self.finish=1
                    ...
                    txt=str(today)+".txt"
                    j=0
                    if self.write==0 and  len(resultpath)>1:
                        ...
                        sql="SELECT  id from AStable where link='%s' "%(self.url)
                        cur.execute(sql)
                        result=cur.fetchall()
                        urlid=result[0][0]
                        sql="UPDATE Ttable SET time='%s' where tid='%s'  and id='%s' " %(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())),self.id,urlid)
                        cur.execute(sql)
                        cur.execute("commit")
                        cur.close()
                        f=open(txt,"a")
                        f.write("from "+self.url+"----"+repr(self.id)+"to "+self.ip+"\n------------------------\n")
                        self.dresult= self.url+" to "+self.ip+"</br>"
                        dresult=" "
                        for i in resultpath:
                            if i[0]=='0':
                                i = i[1:]
                            f.write(str(j+1)+":"+i+"\n") 
                            self.dresult=self.dresult+str(j+1)+":"+i+"</br>"
                            mypath[j]=mypath[j ]-1
                            if mypath[j]<1 :
                                j=j+1
                                if j>99 :
                            ...
                            f1=open("no","a")
                            f1.write(self.url+" "+repr(self.id)+" "+self.ip+"\n") 
                            f1.close()    
                    f.close()
                    self.write=1
            else:
                            f1=open("no","a")
                            f1.write(self.url+" "+str(self.id)+" "+self.ip+"\n") 
                            f1.close()    
